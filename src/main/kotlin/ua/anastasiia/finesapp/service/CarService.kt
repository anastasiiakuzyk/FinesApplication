...
class CarService(val carRepository: CarRepository) {
    ...
    fun getCarByPlate(plate: String): CarResponse =
        carRepository.findByPlate(plate)?.toResponse() ?: throw CarPlateNotFoundException(plate)
    ...
    fun getAllCars(): List<CarResponse> = carRepository.findAll().map { car: Car -> car.toResponse() }
